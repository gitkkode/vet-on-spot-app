...
@Component({
  ...
  template: `
    <div class="portal" [class.portal--menu-open]="menuOpen()">
      <header class="portal-top">
        <a routerLink="/" class="portal-brand" (click)="closeMenu()">
        ...
        <div class="portal-top__actions">
          ...
          >
            ...
            @if (unread() > 0) {
              <span class="portal-top__count">{{ unread() > 9 ? '9+' : unread() }}</span>
            }
          </a>
          <button
            type="button"
      ...
      >
        ...
        <nav class="drawer__nav" aria-label="Main">
          ...
          <a routerLink="/" routerLinkActive="on" [routerLinkActiveOptions]="{ exact: true }" (click)="closeMenu()">
            Home
          </a>
        ...
        <div class="drawer__foot">
          <a class="drawer__book" routerLink="/book/new" [queryParams]="fabPet()" (click)="closeMenu()">
            Book a home visit
          </a>
      ...
      <footer class="portal-foot">
        <div class="portal-foot__inner">
          ...
          <div class="portal-foot__cols">
            <div>
              ...
              <a routerLink="/book/new" [queryParams]="fabPet()">Book a vet</a>
        ...
        <p class="portal-foot__legal">Veterinary hospital · Emergency · Diagnostics · Pharmacy · Wellness</p>
      </footer>

      <a class="fab" routerLink="/book/new" [queryParams]="fabPet()" aria-label="Book a vet">
  ...
  styles: [`
    .portal {
      ...
      background:
        radial-gradient(ellipse 55% 32% at 100% 0%, rgba(253, 74, 41, 0.035) 0%, transparent 60%),
        #FCFCFB;
    }
    ...
    .portal-brand__mark {
      font-family: var(--vos-display); font-weight: 700; font-size: 1.45rem;
      letter-spacing: -0.04em; color: var(--vos-ink);
    }
    .portal-brand__mark .o { color: var(--vos-brand); }
    .portal-brand__sub {
      font-family: var(--vos-mono); font-size: 11px; letter-spacing: 0.14em;
    ...
    .portal-top__menu-btn {
      ...
      border: 1px solid var(--vos-border); background: #fffef9;
      display: inline-flex; align-items: center; justify-content: center;
    ...
    .drawer__book {
      ...
      box-shadow: 0 12px 28px rgba(253, 74, 41, 0.28);
    }
    .drawer__logout {
      min-height: 48px; border-radius: 999px;
    ...
    .portal-foot__label {
      ...
      font-family: var(--vos-mono); font-size: 10px; letter-spacing: 0.16em;
      text-transform: uppercase; color: rgba(255,255,255,0.45); font-weight: 600;
    }
    .portal-foot__cols a {
    ...
    .portal-foot__legal {
      max-width: var(--vos-max); margin: 28px auto 0; padding-top: 18px;
...
export class ShellComponent implements OnInit {
  ...
  async refreshUnread() {
    try {
      const list = await this.api.notifications(true);
      this.unread.set(Array.isArray(list) ? list.length : 0);
    } catch {
      this.unread.set(0);
    }
  }
